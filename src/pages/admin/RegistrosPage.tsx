...
export default function RegistrosPage() {
  ...
  const { data: escolas = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('escolas').select('id, nome, programa, uso_interno');
      if (error) throw error;
            ...
            <div className="flex items-center gap-1">
              {pendente && (
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p>Ação pendente há mais de 7 dias</p>
                  </TooltipContent>
                </Tooltip>
              )}
          ...
          {(() => {
            ...
            return (
              <div className="space-y-4 mt-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="col-span-2">
                    ...
                    <Select value={editAapId} onValueChange={setEditAapId}>
                      ...
                      <SelectContent>
                        {[...profiles].filter(p => (p as any).ativo !== false || p.id === editAapId).sort((a, b) => a.nome.localeCompare(b.nome)).map(p => (
                ...
                {/* Campos genéricos (Observações/Avanços/Dificuldades/Fechamento/Encaminhamentos)
                    removidos da edição de todas as ações. Estados mantidos para preservar dados existentes. */}
              </div>
            );
          })()}
            ...
            {selectedRegistro && (() => {
              ...
              return (
                <ObservacaoAulaGpaForm
                  ...
                  escolaPaiId={selectedRegistro.escola_id}
                  data={selectedRegistro.data}
                  horarioInicio={prog?.horario_inicio || ''}
                  horarioFim={prog?.horario_fim || ''}
